...
export interface VtkViewer2DPanelProps {
  toggleInputId: string;
  frameCount?: number;
  initialFrameIndex?: number;
  onFrameIndexUpdate?: (value: number) => void;
  onColorMapUpdate?: (newMap: ColorMap) => void;
  initialColorMapString?: ColorMapString;
  debug?: boolean;
}
...
>(function VtkViewer2DPanel(
  {
    toggleInputId,
    frameCount = 0,
    initialFrameIndex = 0,
    onFrameIndexUpdate = null,
    onColorMapUpdate = () => {},
    initialColorMapString = {
      controlPoints: [
  ...
  const colorMapRef = useRef<HTMLInputElement | null>(null);
  ...
  useEffect(() => {
    ...
    if (!HSRangeSlider) {
      console.warn("HSRangeSlider not found. Ensure Preline script is loaded.");
      return;
    }
        ...
        <ul className="menu w-full grow">
          ...
          <li className="w-full is-drawer-close:hidden">
            ...
            <div className="border border-gray-300 bg-gray-100 rounded-xl shadow-2xs pt-15 p-6 flex w-full flex-col justify-start dynamicPreline">
              <ColorMapEditor
                ref={colorMapRef}
                onChange={(newMap: ColorMap) => {
                  onColorMapUpdate(newMap); // <-- propagate upward
                }}
                initialColorMap={initialColorMapString}
              />
            </div>
          </li>
        </ul>
